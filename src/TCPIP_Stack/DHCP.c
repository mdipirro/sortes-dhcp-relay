...
typedef struct
{
	...
	struct
	{
	    ...
	    unsigned char bEvent : 1;//Indicates to an external module that the
                                     // DHCP client has been reset, has 
                                     // obtained new parameters via the DHCP 
                                     // client, or has refreshed a lease on 
                                     // existing ones
	    unsigned char bOfferReceived : 1; // Whether or not an offer has 
                                              //been received
	    unsigned char bDHCPServerDetected : 1; // Indicates if a DCHP 
	                             //server has been detected
	    unsigned char bUseUnicastMode : 1;// Indicates if the 
	} bits;
	BYTE val;
    } flags;
    DWORD dwTimer;         // Tick timer value used for triggering future 
                           // events after a certain wait period.
...
***************************************************************************/
    ...
    #define LoadState(v)     v=0; //to avoid irrelevant warnings ML
                                  //V is already 0 in this case
...
/*****************************************************************************
  ...
  Description:
	Resets the DHCP client module, giving up any current lease, knowledge 
	of DHCP servers, etc. for the specified interface.
...
void DHCPInit(BYTE vInterface)
{
    ...
    DHCPClient.flags.bits.bUseUnicastMode = TRUE; // This flag toggles before 
                                                  // use, so this statement 
                                                  // actually means to start 
                                                  // out using broadcast mode.
...
/*****************************************************************************
  ...
  Description:
	Disables the DHCP client for the specified interface by sending the 
	state machine to "SM_DHCP_DISABLED".  If the interface was previously 
	configured by DHCP, the configuration will continue to be used but the 
	module will no longer preform any renewals.

  Precondition:
	None

  Parameters:
	vInterface - Interface number to disable the DHCP client on.   If you 
  ...
  Remarks:
	Since the interface continues using its old configuration, it is 
	possible that the lease may expire and the DHCP server provide the IP 
	to another client.  The application should replace the current IP 
	address and otherconfiguration with static information following a call
        to this function.
***************************************************************************/
void DHCPDisable(BYTE vInterface)
{
...
/*****************************************************************************
  ...
  Parameters:
	vInterface - Interface number to query. If you only have one 
		interface, specify 0x00.
...
/*****************************************************************************
  ...
  Summary:
	Determins if the DHCP client on the specified interface has changed
	states or refreshed its IP address lease.

  Description:
	Determins if the DHCP client on the specified interface has changed 
	states or refreshed its IP address lease.  This function can be used 
	to determine when to update an LCD or other display whenever the DHCP 
	assigned IP address has potentially changed.
  ...
  Returns:
	TRUE - The IP address lease have been reliquished (due to 
		reinitilization), obtained (first event), or renewed since 
		the last call to DHCPStateChanged().
	FALSE - The DHCP client has not detected any changes since the last 
		call to DHCPStateChanged().
***************************************************************************/
BOOL DHCPStateChanged(BYTE vInterface)
{
    LoadState(vInterface);
...
/*****************************************************************************
  ...
  Summary:
	Determines if the DHCP client on the specified interface has seen a 
	DHCP server.

  Description:
	Determins if the DHCP client on the specified interface has seen a 
	DHCP server.
	...
	switch(DHCPClient.smState)
	{
	    ...
	    case SM_DHCP_GET_SOCKET:
	        ...
	        DHCPClient.hDHCPSocket = 
                    UDPOpen(DHCP_CLIENT_PORT, NULL, DHCP_SERVER_PORT);
...
static void _DHCPSend(BYTE messageType, BOOL bRenewing)
{
    ...
    UDPPut(0);                                  // secs[0]
    UDPPut(0);                                  // secs[1]
    UDPPut(DHCPClient.flags.bits.bUseUnicastMode ? 0x00: 0x80);
    ...
    if(!bRenewing) AppConfig.MyIPAddr.Val = 0x00000000;
    UDPFlush();
